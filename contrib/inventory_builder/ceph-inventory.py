...
ROLES = ['all', 'kube-master', 'kube-node', 'ceph-mon', 'ceph-osd', 'etcd', 'k8s-cluster',
         'calico-rr']
...
class KubesprayInventory(object):
    ...
    def __init__(self, changed_hosts=None, config_file=None):
        ...
        if changed_hosts:
            ...
            if len(self.hosts) >= SCALE_THRESHOLD:
                ...
                self.set_ceph_mon(list(self.hosts.keys())[etcd_hosts_count:5])
            else:
                self.set_kube_master(list(self.hosts.keys())[:3])
                self.set_ceph_mon(list(self.hosts.keys())[:3])
            self.set_kube_node(self.hosts.keys())
            self.set_ceph_osd(self.hosts.keys())
            if len(self.hosts) >= SCALE_THRESHOLD:
    ...
    def set_ceph_mon(self, hosts):
        for host in hosts:
            self.add_host_to_group('ceph-mon', host)

    def set_ceph_osd(self, hosts):
        for host in hosts:
            self.add_host_to_group('ceph-osd', host)
